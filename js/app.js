...
if(txDetailNumber){
  ...
  document.getElementById("reconciliationForm")?.addEventListener("submit",async e=>{e.preventDefault();const method=document.getElementById("verificationMethod").value,date=document.getElementById("verificationDate").value;if(!method||!date){alert("Select a verification method and date.");return;}try{const {data:{user}}=await supabaseClient.auth.getUser();const ref=document.getElementById("verificationReference").value.trim()||null;const notes=document.getElementById("reconciliationNotes").value.trim()||null;let payload={organisation_id:tx.organisation_id,transaction_id:tx.id,verification_method:method,verification_date:date,verification_reference:ref,notes,reconciled_by:user?.id||null};let {error}=await supabaseClient.from("reconciliations").insert(payload);if(error){delete payload.reconciled_by;payload.created_by=user?.id||null;({error}=await supabaseClient.from("reconciliations").insert(payload));}if(error)throw error;const {error:u}=await supabaseClient.from("transactions").update({reconciliation_status:"reconciled"}).eq("id",tx.id);if(u)throw u;close();await loadTxDetail();const toast=document.getElementById("reconciliationToast");if(toast){toast.hidden=false;setTimeout(()=>toast.hidden=true,2500);}}catch(err){console.error(err);alert(err.message||"Unable to reconcile transaction.");}});
